...
def course_recommender(course_list):
    st.subheader("**DIFFERENT JOB ROLES**")
    c = 0
    rec_course = []
    no_of_reco = st.slider('Choose Number of Job Roles Recommendations:', 1, 10, 4)
    random.shuffle(course_list)
    for c_name in course_list:
        c += 1
        st.markdown(f"({c}) [{c_name}]")
...
def run():
    ...
    table_sql = "CREATE TABLE IF NOT EXISTS " + DB_table_name + """
                    (ID INT NOT NULL AUTO_INCREMENT,
                     ...
                     Recommended_JOB VARCHAR(600) NOT NULL,
                     PRIMARY KEY (ID));
                    """
